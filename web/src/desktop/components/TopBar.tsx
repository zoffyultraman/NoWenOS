...
import {
  HardDrive, Search, User, LogOut, Globe, Sun, Moon,
} from "lucide-react";
...
export function TopBar() {
  ...
  return (
    <header
      className="fixed top-0 left-0 right-0 z-[9999] flex h-8 items-center justify-between border-b border-border bg-background/90 px-3 backdrop-blur-md select-none"
    ...
    >
      ...
      {/* Center: Search hint */}
      <div
        className="flex items-center gap-1.5 rounded-md bg-muted/50 px-2.5 py-0.5 text-[10px] text-muted-foreground"
        style={{ WebkitAppRegion: "no-drag" } as React.CSSProperties}
      >
        <Search className="h-3 w-3" />
        <span>{t("common.search")} &middot; Ctrl+K</span>
      </div>
